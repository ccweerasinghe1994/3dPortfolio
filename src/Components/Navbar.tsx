        ...
        <nav className={`${styles.paddingX} w-full flex items-center py-5 fixed top-0 z-20 bg-primary`}>
            <div className="w-full flex justify-between items-center max-w-7xl mx-auto">
                <Link to={'/'} className={'flex items-center gap-2'} onClick={() => {
                    ...
                    <p className={'text-white text-[18px] cursor-pointer flex'}>Chamara <span
                        className={'sm:block hidden'}>| Weerasinghe</span>
                    </p>
                </Link>
                        ...
                        <ul className={'list-none flex flex-col items-start justify-end gap-4 '}>
                            {
                                navLinks.map((link) => {
                                    return (
                                        <li key={link.id} className={`
                                   ${active === link.title ? "text-white" : 'text-secondary'} font-popins font-medium cursor-pointer text-[16px]`}
                                            onClick={() => {
